...
```{r}
hive_0615 <- read.csv("data/0615_VideoAnalysis2.csv")
hive_0616 <- read.csv("data/0616_VideoAnalysis2.csv")
hive_0617 <- read.csv("data/0617_VideoAnalysis2.csv")
...
select_hive_columns <- . %>%
  select(Colony, 
         ...
         Bout_13,
         Bout_14) %>%
  filter(!is.na(Bee) & Bee != "") # removes rows representing array visits where
                                  # we didn't observe a subsequent dance
```
...
hive_summary <- combined_hive_df %>%
  ...
  summarize(Dances = sum(Danced, na.rm = T), 
            Dance_rate = ifelse(sum(Danced, na.rm = T) > 0 & mean(Duration) < 0, 1, sum(Danced, na.rm = T) / mean(Duration/60)),
