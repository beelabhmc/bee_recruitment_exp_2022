...
```{r}
convert_bee_IDs <- function(df){
  
  for (row in 1:length(df$Bee)){
    
    df$Bee[row] <- gsub(pattern = "-",
                        replacement = ".",
                        x = df$Bee[row])
  }
...
```{r}
for (i in 1:length(hive_dfs)){
  hive_dfs[[i]] <- convert_bee_IDs(hive_dfs[[i]])
  list2env(hive_dfs,.GlobalEnv)
}
```
...
hive_summary <- combined_hive_df %>%
  group_by(Colony, Date, Scent, Site, Density, Flower_number, Bee) %>%
...
# Step : Add trip numbers and intertrip intervals (between previous trip exit and current trip entrance w NA for first trip) to hive data frame based on bee ID, date, and entrance time

# Step : Write out list of improbable/impossible intertrip intervals

# Step : Create a summary data frame for each bee ID sighted at array with total number of dances performed

# Step : Write out the processed hive data frame

# Step : Write out the bee ID summary data frame
