...
model_dance_olrs <- glmmTMB(Dances ~ Density + Flower_number + Scent +
                          (1|Date) + (1|Trial) + (1|Bee) + (1|ObsID),
                        family = "poisson", 
                        data = bees_df)

# negative binomial with linear parameterization
model_dance_nb1 <- glmmTMB(Dances ~ Density + Flower_number + Scent +
                         (1|Date) + (1|Trial) + (1|Bee), 
                         family = "nbinom1", 
                         data = bees_df)

# negative binomial with quadratic parameterization
model_dance_nb2 <- glmmTMB(Dances ~ Density + Flower_number + Scent +
                          (1|Date) + (1|Trial) +  (1|Bee), 
                         family="nbinom2", 
                         data = bees_df)

# zero-inflated model with constant zero probability
model_dance_zip <- glmmTMB(Dances ~ Density + Flower_number + Scent +
                          (1|Date) + (1|Trial) + (1|Bee),  
                          zi = ~1,
                          family="poisson", 
                          data = bees_df)
...
model_dance_zip2 <- glmmTMB(Dances ~ Density + Flower_number + Scent +
                          (1|Date) + (1|Trial) + (1|Bee),  
                          zi = ~.,
                          family="poisson", 
                          data = bees_df)
...
```{r}
model_runs_olrs <- glmmTMB(Total_runs ~ Density + Flower_number + Scent +
                          (1|Date) + (1|Trial) + (1|Bee) + (1|ObsID),
                        family = "poisson", 
                        data = dances_df)

model_runs_nb1 <- glmmTMB(Total_runs ~ Density + Flower_number + Scent +
                          (1|Date) + (1|Trial) + (1|Bee), 
                         family = "nbinom1", 
                         data = dances_df)

model_runs_nb2 <- glmmTMB(Total_runs ~ Density + Flower_number + Scent +
                          (1|Date) + (1|Trial) +  (1|Bee), 
                         family="nbinom2", 
                         data = dances_df)
```
