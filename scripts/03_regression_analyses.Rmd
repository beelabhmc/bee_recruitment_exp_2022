...
library("AICcmodavg") # for making delta AIC comparison table
...
bees_df <- read.csv("cleaned_data/01_per_bee_summary.csv") %>%
  mutate(ObsID = row_number())
...
model_runs_zip <- glmmTMB(Total_runs ~ Density + Flower_number + 
                          (1|Date) + (1|Trial) + (1|Bee),  
                          zi = ~1,
                          family="poisson", 
                          data = dances_df)
