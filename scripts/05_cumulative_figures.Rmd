...
# Step : Add a column so we can get cumulative totals per minute of each trial
...
# Step : Combine cumulative_df and cumulative_array_df so we can scale by number of bees that
...
Unique bees who danced scaled by the cumulative number of bees who had visited the feeder
...
Dances performed scaled by the cumulative number of bees that had visited the array
...
combined_df %>%
  ...
  scale_linetype_manual(values = c("twodash", "solid", "dotted")) +
  geom_line(linewidth = 1) +
  labs(y = "Cumulative dances per bee") +
  facet_grid(Trial~Treatment, scales = "free_y") +  # trials had very different totals
  theme_light(base_size = 16)
...
combined_df %>%
  ...
  scale_linetype_manual(values = c("twodash", "solid", "dotted")) +
  geom_line(linewidth = 1) +
  labs(y = "Cumulative waggle runs per bee") +
  facet_grid(Trial~Treatment, scales = "free_y") +  # trials had very different totals
  theme_light(base_size = 16)
...
Unique bees who visited flower array- this shows how many bees could possibly have decided to contribute to the number of dances and waggle runs.
