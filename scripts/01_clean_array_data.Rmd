...
# Step : Convert any text in cells to 1s
...
```{r}
for (i in 1:length(array_dfs)){
  array_dfs[[i]] <- fix_ids(array_dfs[[i]])
  list2env(array_dfs,.GlobalEnv)
}
```
...
first_sighting <- . %>%
  ...
  mutate(Duration = 60 - Decimal_time)
```

```{r}
for (i in 1:length(array_dfs)){
  array_dfs[[i]] <- first_sighting(array_dfs[[i]])
  list2env(array_dfs,.GlobalEnv)
}
```

Correct Trial 1 duration values (2 hours instead of 1):
